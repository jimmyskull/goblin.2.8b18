...
int main(int ParamCount,const char *ParamStr[])
{
    ...
    try
    {
        ...
        if (gob)
        {
            XLP = ModLP->ReadInstance(gobIn,*CT);
            cout << "...Problem read from " << gobIn << endl;
        }
        else
        {
            XLP = ModLP->NewInstance(1,1,1,goblinILPWrapper::MINIMIZE,*CT);
        ...
        if (ret==0)
        {
            ...
            else
            {
                ...
                XLP -> Write(basFile,goblinILPWrapper::BAS_CPLEX);
            }
        }
