...
int main(int ParamCount,const char *ParamStr[])
{
    ...
    {
        cout << "...No source node specified!" << endl << endl;
        return 1;
    }
    ...
    {
        diGraph *H = new diGraph(fileIn,*CT);

        int t = CT->FindParam(ParamCount,ParamStr,"-t");
        TNode vt = NoNode;
        if (t)
        {
            vt = atoi(ParamStr[t+1]);
            cout << "Source node s := " << vs << endl;
            cout << "Target node t := " << vt << endl;
        }
        else
        {
            cout << "...No target node specified!" << endl << endl;
            return 1;
        }

        if (vs==vt || vs>=H->N() || vt>=H->N())
        {
            cout << "...Bad choice of source/target node!" << endl << endl;
            return 1;
        }

        G = new splitGraph(*H,vs,vt);
        vs = G->N()-1;
