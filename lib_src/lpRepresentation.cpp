...
goblinILPWrapper::TVarType goblinLPSolver::VarType(TVar i)
    const throw(ERRange)
{
    #if defined(_FAILSAVE_)
    ...
    throw(ERRange,ERRejected)
{
    ...
    else
    {
        if (newLabel==NULL || strcmp(newLabel,"")==0 ||
            strcmp(newLabel,goblinILPWrapper::VarLabel(i,OWNED_BY_RECEIVER))==0)
        {
            if (varLabel!=NULL)
            {
                delete[] varLabel[i];
    ...
    if (varIndex!=NULL)
    {
        ...
        else varIndex -> ChangeKey(goblinILPWrapper::VarLabel(i),i,NoIndex,OWNED_BY_SENDER);
    }
}
    ...
    throw(ERRange,ERRejected)
{
    ...
    {
        if (newLabel==NULL || strcmp(newLabel,"")==0 ||
            strcmp(newLabel,goblinILPWrapper::RestrLabel(i,OWNED_BY_RECEIVER))==0)
        {
            if (restrLabel!=NULL)
            {
                delete[] restrLabel[i];
    ...
    if (restrIndex!=NULL)
    {
        ...
        else restrIndex -> ChangeKey(goblinILPWrapper::RestrLabel(i),i,NoIndex,OWNED_BY_SENDER);
    }
}
