...
TPoolEnum attributePool::ReadAttribute(goblinRootObject& X,goblinImport& F) throw()
{
    ...
    switch (table[thisToken].arrayType)
    {
        ...
        case TYPE_STRING:
        {
            attribute<TString>* A = MakeAttribute<TString>(X,thisToken,ATTR_REQ_SINGLETON);
            F.ReadTupleValues(table[thisToken].arrayType,A->Size(),A->GetArray());
            break;
        }
        case TYPE_VAR_INDEX:
    ...
    if (   table[thisToken].arrayType!=TYPE_SPECIAL
       ...
       )
    {
        ...
        else
        {
            ...
            #define _STATEMENT A -> SetConstant(A->GetValue(0));
            #include "switchBaseTypes.h"
            #undef _STATEMENT
        }
    }
...
/*
    ...
    while (I != attributes.end())
    {
        ...
        #define _STATEMENT cout << "  size = " << A->Size() << endl;
        #include "switchBaseTypes.h"
        #undef _STATEMENT
...
void attributePool::WriteAttribute(const goblinRootObject& X,goblinExport& F,TPoolEnum token) const throw()
{
    ...
    {
        WriteAttribute(X,F,_attribute,table[token].arrayType,table[token].tokenLabel);
    }
    else
    {
...
void attributePool::WriteAttribute(const goblinRootObject& X,goblinExport& F,
    attributeBase* _attribute,TBaseType attributeType,
    const char* attributeLabel) const throw()
{
    switch (attributeType)
    {
        ...
        case TYPE_STRING:
        {
            attribute<TString>* A = static_cast<attribute<TString>*>(_attribute);
            F.WriteAttribute(A->GetArray(),attributeLabel,A->Size(),TString(NULL));
            break;
        }
        case TYPE_VAR_INDEX:
        {
...
void attributePool::WritePool(const goblinRootObject& X,goblinExport& F,const char* poolLabel) const throw()
{
    ...
    while (I != attributes.end())
    {
        ...
        if (table[token].arrayType!=TYPE_SPECIAL)
        {
            WriteAttribute(X,F,*I,table[token].arrayType,table[token].tokenLabel);
        }
...
void attributePool::ReserveItems(TArrayDim attributeType,TIndex capacity) throw(ERRange)
{
    ...
    while (I != attributes.end())
    {
        ...
        {
            #define _STATEMENT A -> ReserveItems(capacity);
            #include "switchBaseTypes.h"
            #undef _STATEMENT
        }
...
void attributePool::AppendItems(TArrayDim attributeType,TIndex numItems) throw()
{
    ...
    while (I != attributes.end())
    {
        if (table[(*J)].arrayDim == attributeType)
        {
            #define _STATEMENT A -> AppendItems(numItems);
            #include "switchBaseTypes.h"
            #undef _STATEMENT
        }
...
void attributePool::EraseItems(TArrayDim attributeType,TIndex numItems) throw(ERRange)
{
    ...
    while (I != attributes.end())
    {
        if (table[(*J)].arrayDim == attributeType)
        {
            #define _STATEMENT A -> EraseItems(numItems);
            #include "switchBaseTypes.h"
            #undef _STATEMENT
        }
...
void attributePool::SwapItems(TArrayDim attributeType,TIndex i1,TIndex i2) throw(ERRange)
{
    ...
    while (I != attributes.end())
    {
        if (table[(*J)].arrayDim == attributeType)
        {
            #define _STATEMENT A -> SwapItems(i1,i2);
            #include "switchBaseTypes.h"
            #undef _STATEMENT
        }
