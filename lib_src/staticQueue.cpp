...
char* staticQueue<TItem,TKey>::Display() const throw()
{
    ...
    else
    {
        ...
        sprintf(this->CT.logBuffer,"%ld (last in)",i);
...
template <class TItem,class TKey>
void staticQueue<TItem,TKey>::Insert(TItem w,TKey alpha,TOptInsert mode) throw(ERRange,ERCheck)
{
    ...
    if (next[w]!=n)
    {
        ...
        sprintf(this->CT.logBuffer,"%ld is already on the queue",w);
...
template class staticQueue<TNode,TFloat>;

#if defined(_BIG_ARCS_)

template class staticQueue<TArc,TFloat>;
