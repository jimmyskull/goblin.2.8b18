...
void abstractGraph::ChinesePostman(bool adjustUCap) throw(ERRejected)
{
    ...
    if (adjustUCap)
    {
        ...
        LogEntry(LOG_METH,"Computing minimum Eulerian supergraph...");
    }
    else
    {
        X = NULL;
        LogEntry(LOG_METH,"Computing maximum Eulerian subgraph...");
    }
    ...
    CloseFold(ModPostman,NO_INDENT);
}
...
void abstractDiGraph::ChinesePostman(bool adjustUCap) throw(ERRejected)
{
    ...
    for (TNode v=0;v<n;v++)
    {
        if (Divergence(v)>0)
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"Divergence(%ld) = %g",v,Divergence(v));
    ...
    if (CT.logRes)
    {
        sprintf(CT.logBuffer,"...Problem has %ld unbalanced nodes",n0);
    ...
    InitProgressCounter(n1+n+deficiency);
    SetProgressNext(1);
    ...
    for (TNode u=0;u<n;u++)
    {
        if (map1[u]!=NoNode)
        {
            ShortestPath(SPX_DIJKSTRA,SPX_PLAIN,u);
    ...
    for (TArc a=0;a<G->M();a++)
    {
        if (G->Sub(2*a)>0)
        {
            ...
            ShortestPath(SPX_DIJKSTRA,SPX_REDUCED,u);
...
bool abstractMixedGraph::EulerCycle(TArc* pred) throw(ERRejected)
{
    if (MaxUCap()>1.0)
        Error(ERR_REJECTED,"EulerCycle","Capacity bounds must be all one");
    ...
    OpenFold(ModHierholzer,SHOW_TITLE);
    ...
    if (CT.logMeth>1)
    {
        sprintf(CT.logBuffer,"Initial cycle: (%ld[%ld]",StartNode(a),a);
        LH = LogStart(LOG_METH2,CT.logBuffer);
    }
    ...
    while (true)
    {
        ...
        if (I.Active(u))
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"%ld[%ld]",u,a);
                LogAppend(LH,CT.logBuffer);
            }
    ...
    while (isEulerian && a!=0)
    {
        ...
        if (I.Active(u))
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"Inserting before %ld: (%ld[%ld]",
                    a,StartNode(a2),a2);
                LH = LogStart(LOG_METH2,CT.logBuffer);
            }
            ...
            while (true)
            {
                ...
                if (I.Active(u))
                {
                    ...
                    if (CT.logMeth>1)
                    {
                        sprintf(CT.logBuffer,"%ld[%ld]",u,a2);
                        LogAppend(LH,CT.logBuffer);
                    }
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"%ld)",EndNode(a2));
                LogEnd(LH,CT.logBuffer);
            }
    ...
    if (isEulerian)
    {
        LogEntry(LOG_RES,"...Graph is Eulerian");
    }
    else
    {
        LogEntry(LOG_RES,"...Graph is not Eulerian");
    }
