...
{
    X = new diGraph(*Node.X,OPT_CLONE);
    H = X->Investigate();
...
void branchAsyTSP::SetCandidateGraph(int nCandidates) throw()
{
    ...
    CT.SuppressLogging();
    diGraph* Y = new diGraph(G,OPT_CLONE);
...
branchNode<TArc,TFloat>::TBranchDir branchAsyTSP::DirectionConstructive(TArc a)
    throw(ERRange)
{
    #if defined(_FAILSAVE_)

    if (a>=n) NoSuchArc("DirectionConstructive",a);
...
branchNode<TArc,TFloat>::TBranchDir branchAsyTSP::DirectionExhaustive(TArc a)
    throw(ERRange)
{
    #if defined(_FAILSAVE_)

    if (a>=n) NoSuchArc("DirectionExhaustive",a);
...
branchNode<TNode,TFloat> *branchAsyTSP::Clone() throw()
{
...
TFloat branchAsyTSP::LocalSearch() throw()
{
    ...
    // It can happen that found tour is bettre than every tour in
    // the candidate graph
    if (objective>=LowerBound(TimerTsp))
    {
        SetUpperBound(TimerTsp,objective);
    }
    else
    {
        SetUpperBound(TimerTsp,LowerBound(TimerTsp));
    }
...
{
    LogEntry(LOG_METH,"ATSP Branch and Bound...");
    OpenFold(ModTSP);
    ...
    branchScheme<TArc,TFloat> scheme(masterNode,upperBound,level);

    CloseFold(ModTSP);

    if (scheme.savedObjective!=InfFloat)
    {
        SetUpperBound(TimerTsp,scheme.savedObjective);
    ...
    throw(ERRange,ERRejected)
{
    ...
    LogEntry(LOG_METH,"Starting TSP solver...");
    OpenFold(ModTSP);
    ...
    if (pred)
    {
        ...
        if (i==n)
        {
            ...
            SetUpperBound(TimerTsp,savedLength);
        }
    ...
    if (!IsDense())
    {
        ...
        SetLowerBound(TimerTsp,ceil(lower-CT.epsilon));

        if (lower==InfFloat)
        {
            LogEntry(LOG_RES,"...Graph is non-Hamiltonian");
            CloseFold(ModTSP);
            return InfFloat;
        }
    ...
    if (methRelax1>=TSP_RELAX_1TREE)
    {
        ...
        try
        {
            ...
            if (methRelax2<TSP_RELAX_1TREE) SetProgressNext(1);
    ...
    if (methRelax2>=TSP_RELAX_1TREE && lower<bestUpper)
    {
        ...
        SetUpperBound(TimerTsp,bestUpper);
    }
    ...
    TFloat& bestUpper,bool branchAndBound) throw(ERRange)
{
    ...
    OpenFold(ModSubgradOptTSP,SHOW_TITLE);
    ...
    sprintf(CT.logBuffer,"Computing minimum %ld-tree...",root);
    ...
    if (method==TSP_RELAX_1TREE)
    {
        ...
        SetLowerBound(TimerTsp,ceil(thisBound-CT.epsilon));
    ...
    if (maxPi>0 && t0>maxPi/10) t0 = maxPi/10;
    TFloat t = t0;
           ...
           && CT.SolverRunning())
    {
        ...
        {
            SetProgressCounter( (log(t0/tMin)-log(t/tMin)) / log(t0/tMin) );
        }
        ...
        if (CT.logMeth>=2)
        {
            sprintf(CT.logBuffer,"Nodes with incorrect degrees: %ld",dev);
            LogEntry(LOG_METH2,CT.logBuffer);
        }
        ...
        else if (thisBound>bestLower+CT.epsilon)
        {
            ...
            if (!branchAndBound)
            {
                ...
                SetLowerBound(TimerTsp,ceil(bestLower-CT.epsilon));
            }
...
TFloat denseDiGraph::TSP_Heuristic(THeurTSP method,TNode root) throw(ERRange,ERRejected)
{
    ...
    OpenFold(ModTSP,NO_INDENT);

    LogEntry(LOG_METH,"Computing heuristic tour...");
...
TFloat abstractMixedGraph::TSP_Heuristic(THeurTSP method,TNode root) throw(ERRange,ERRejected)
{
    ...
    LogEntry(LOG_METH,"Transforming to dense digraph...");
    ...
    int swapSubgraph = CT.subgraph;
    CT.methSolve = 1;
    CT.subgraph = 0;

    TFloat savedLowerBound = LowerBound(TimerTsp);
    ResetBounds(TimerTsp);
    ...
    CT.subgraph = swapSubgraph;

    ResetBounds(TimerTsp,savedLowerBound,InfFloat);
    SetUpperBound(TimerTsp,length);
    ...
    {
        LogEntry(LOG_RES,"Tour does not map to the original graph");
    }
...
{
    OpenFold(ModRandomTour,SHOW_TITLE);
    ...
    if (CT.logMeth>1)
    {
        sprintf(CT.logBuffer,"Constructed tour: 0,%ld",x);
...
TFloat abstractMixedGraph::TSP_HeuristicInsert(THeurTSP method,TNode r) throw(ERRange,ERRejected)
{
    if (r==NoNode)
    {
        OpenFold(ModFarthestInsert,NO_INDENT);

        #if defined(_PROGRESS_)

        InitProgressCounter(n*n);
        ...
        CloseFold(ModFarthestInsert,NO_INDENT);
    ...
    OpenFold(ModFarthestInsert,SHOW_TITLE);
    ...
    if (CT.logMeth>1)
    {
        sprintf(CT.logBuffer,"Root node: %ld",r);
        LogEntry(LOG_METH2,CT.logBuffer);
    ...
    for (TNode i=0;i<n-1;i++)
    {
        ...
        if (CT.logMeth>1)
        {
            sprintf(CT.logBuffer,"%ld,",optNode);
            LogAppend(LH,CT.logBuffer);
        }
...
TFloat abstractMixedGraph::TSP_HeuristicTree(TNode r) throw(ERRange,ERRejected)
{
    if (r==NoNode)
    {
        OpenFold(ModTreeApproxTSP,SHOW_TITLE);
