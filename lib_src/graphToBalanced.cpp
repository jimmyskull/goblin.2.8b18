...
void graphToBalanced::ExportDecomposition() throw()
{
    ...
    {
        if (dist[2*v]==InfFloat && dist[2*v+1]==InfFloat)
        {
            nodeColour[v] = 2;
        }
        else if (dist[2*v]!=InfFloat)
        {
            nodeColour[v] = 1;
        }
        else
...
bool graphToBalanced::HiddenArc(TArc a) const throw(ERRange)
{
    if (a<2*m)
    {
        const TFloat* dist = GetDistanceLabels();
...
TFloat graphToBalanced::CancelOdd() throw()
{
    ...
    LogEntry(LOG_METH,"Cancelling odd length cycles...");
    ...
    if (CT.logRes)
    {
        sprintf(CT.logBuffer,"%ld odd length cycles eliminated",k1+k2);
        LogEntry(LOG_RES,CT.logBuffer);
        sprintf(CT.logBuffer,"Flow value decreases by %ld units",k2);
        LogEntry(LOG_RES,CT.logBuffer);
    }
    ...
    if (k2>1)
    {
        ...
        val = BNSAndAugment(Source());
    }
    else
    {
        val = UpperBound(TimerMaxBalFlow);
