...
void abstractMixedGraph::Layout_ForceDirected(TOptFDP method,int spacing)
    throw(ERRejected)
{
    ...
    LogEntry(LOG_METH,"Force directed drawing...");
    ...
    switch (method)
    {
        ...
        case FDP_LAYERED_RESTR:
        {
            ...
            for (TNode v=0;v<G.N();v++)
            {
                ...
                {
                    X -> SetC(G.OriginalNode(v),i,G.C(v,i));
                }
            }
            ...
            for (TArc a=0;a<m;a++)
            {
                ...
                for (TDim i=0;i<G.Dim();i++)
                {
                    X -> SetC(x,i,C(y,i) + ((i==0) ? CT.fineSep : 0));
                }
            }
...
void abstractMixedGraph::Layout_SpringEmbedder(int spacing)
    throw(ERRejected)
{
    #if defined(_FAILSAVE_)
    ...
    OpenFold(ModSpringEmbedder,SHOW_TITLE);

    #if defined(_PROGRESS_)

    InitProgressCounter(1000000);
    ...
    if (spacing>0) CT.nodeSep = spacing;

    double gamma = 1.0/double(CT.nodeSep);
    double delta = 1.0/double(CT.nodeSep);
    ...
    while (CT.SolverRunning() && stepLength>0.1*log(double(n)))
    {
        ...
        if (stepValid)
        {
            ...
            SetProgressCounter((unsigned long)(combEstimate*1000000));
...
void abstractMixedGraph::Layout_GEMDrawing(TOptFDP options,int spacing)
    throw(ERRejected)
{
    #if defined(_FAILSAVE_)
    ...
    TFloat delta = CT.nodeSep;   // Optimal edge length
    TFloat gamma = CT.nodeSep;   // Edge separation constant
    ...
    while (CT.SolverRunning() && tGlobal>tMin*n && stepCount<n*300)
    {
        ...
        SetProgressCounter((unsigned long)(
    ...
    if (CT.logRes)
    {
        sprintf(CT.logBuffer,"...%ld iterations in total",stepCount-1);
        LogEntry(LOG_RES,CT.logBuffer);
...
void abstractMixedGraph::Layout_LayeredFDP(TOptFDP method,int spacing)
    throw(ERRejected)
{
    #if defined(_FAILSAVE_)
    ...
    OpenFold(ModLayeredFDP,SHOW_TITLE);
