...
goblinILPWrapper::TRestrType goblinLPSolver::RestrType(TRestr i)
    const throw(ERRange)
{
    ...
    const throw(ERRejected)
{
    ...
    OpenFold(ModLpPivoting,NO_INDENT);
    ...
    if (!baseValid)
    {
        ...
        LogEntry(LOG_METH2,"...Basis information is evaluated");
    }

    SolutionUpdate();

    CloseFold(ModLpPivoting,NO_INDENT);
}
    ...
    const throw(ERRange,ERRejected)
{
    ...
    LogEntry(LOG_METH2,"...Basis inverse is updated");
    ...
    const throw(ERRejected)
{
    ...
    LogEntry(LOG_METH2,"...Solutions are updated");
