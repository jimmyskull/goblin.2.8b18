...
void surfaceGraph::ConfigDisplay(goblinController &CFG) const throw()
{
    CFG.nodeLabels = 3;
    CFG.arcLabels = 5;
}
...
void surfaceGraph::Traverse(TArc* pred,TArc aIn,TArc aOut) throw()
{
    ...
    if (v!=StartNode(aOut))
    {
        sprintf(CT.logBuffer,"Mismatching end nodes of arcs %ld,%ld",aIn,aOut);
        Error(MSG_WARN,"Traverse",CT.logBuffer);
        sprintf(CT.logBuffer,"Found end nodes: %ld,%ld",v,StartNode(aOut));
    ...
    if (v<n0)
    {
        ...
        if (CT.logMeth>1)
        {
            sprintf(CT.logBuffer,"pred[%ld] = %ld",v,aIn);
    ...
    else
    {
        ...
        if (CT.logMeth>1)
        {
            sprintf(CT.logBuffer,
                "Prop of blossom %ld changes to %ld",v,bprop[b]);
            LogEntry(LOG_METH2,CT.logBuffer);
        }
...
void surfaceGraph::Expand(TArc* pred,TArc aIn,TArc aOut) throw()
{
    ...
    if (CT.logMeth>1)
    {
        sprintf(CT.logBuffer,"Expand(%ld,%ld) started...",aIn,aOut);
        LogEntry(LOG_METH2,CT.logBuffer);
        OpenFold();
    }
...
void surfaceGraph::ExpandAndAugment(TArc aIn,TArc aOut) throw()
{
    #if defined(_LOGGING_)
    ...
    if (CT.logMeth>1)
    {
        sprintf(CT.logBuffer,"pred[%ld] = %ld",v,aOut);
        LogEntry(LOG_METH2,CT.logBuffer);
        CloseFold();
    }
...
void surfaceGraph::CheckDual() throw()
{
    for (TArc a=0;a<2*m;a++)
    {
        ...
        if (BalCap(a)>0 && modlength!=NULL && ModLength(a)!=ret)
        {
            ...
            sprintf(CT.logBuffer,"Arc %ld with end nodes %ld and %ld",
                a,G.StartNode(a),G.EndNode(a));
...
TArc surfaceGraph::FindSupport(TFloat* dist,TNode s,TArc a,
    dynamicStack<TNode,TFloat> &Support) throw()
{
    ...
    if (CT.logMeth>1)
    {
        sprintf(CT.logBuffer,"Q[%ld] = %ld",y^1,Q[y^1]);
        LogEntry(LOG_METH2,CT.logBuffer);
    }
    ...
    while (x!=y)
    {
        if (dist[x]>dist[y])
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"Q[%ld] = %ld",x,Q[x]);
                LogEntry(LOG_METH2,CT.logBuffer);
                sprintf(CT.logBuffer,"Q[%ld] = %ld",x^1,Q[x^1]);
                LogEntry(LOG_METH2,CT.logBuffer);
            }
        ...
        else
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"Q[%ld] = %ld",y,Q[y]);
                LogEntry(LOG_METH2,CT.logBuffer);
                sprintf(CT.logBuffer,"Q[%ld] = %ld",y^1,Q[y^1]);
                LogEntry(LOG_METH2,CT.logBuffer);
            }
...
TFloat surfaceGraph::ComputeEpsilon(TFloat* dist) throw()
{
    ...
    for (TArc a=0;a<2*m && epsilon>0.5;a++)
    {
        ...
        {
            sprintf(CT.logBuffer,"Negative modified length %g",ModLength(a));
            Error(MSG_WARN,"ComputeEpsilon",CT.logBuffer);
            sprintf(CT.logBuffer,"Implicit modified length %g",RModLength(a));
            Error(MSG_WARN," ",CT.logBuffer);
            sprintf(CT.logBuffer,"Arc %ld with end nodes %ld and %ld",a,u,v);
    ...
    if (CT.logMeth>1)
    {
        ...
        if (epsilon>0.5 || epsilon1==0.5)
            sprintf(CT.logBuffer,"epsilon1 = %g",epsilon1);
        else
            sprintf(CT.logBuffer,"epsilon1 <= %g",epsilon1);
        ...
        if (epsilon>0.5 || epsilon2==0.5)
            sprintf(CT.logBuffer,"epsilon2 = %g",epsilon2);
        else
            sprintf(CT.logBuffer,"epsilon2 <= %g",epsilon2);
        ...
        if (epsilon>0.5 || epsilon3==0.5)
            sprintf(CT.logBuffer,"epsilon3 = %g",epsilon3);
        else
            sprintf(CT.logBuffer,"epsilon3 <= %g",epsilon3);
        LogEntry(LOG_METH2,CT.logBuffer);
...
void surfaceGraph::PrimalDual0(TNode s) throw(ERRange)
{
    ...
    staticQueue<TArc,TFloat> Queue(2*m,CT);
    ...
    while (epsilon<InfFloat)
    {
        ...
        while (searching && epsilon<InfFloat)
        {
            ...
            while (searching && !Queue.Empty())
            {
                ...
                if (CT.logMeth>1)
                {
                    sprintf(CT.logBuffer,"Searching node %ld...",u);
                    LogEntry(LOG_METH2,CT.logBuffer);
                    OpenFold();
                }
                ...
                while (searching && Top(u) && I.Active(u))
                {
                    ...
                    if (dist[cv]==InfFloat)
                    {
                        if (dist[v]==InfFloat && BalCap(a)>0 && ModLength(a)==0)
                        {
                            ...
                            {
                                sprintf(CT.logBuffer,
                                    "Prop %ld with end node %ld found",a,v);
                                LogEntry(LOG_METH2,CT.logBuffer);
                            }
                    ...
                    {
                        sprintf(CT.logBuffer,
                            "Petal %ld with end node %ld found",a,v);
                        LogEntry(LOG_METH2,CT.logBuffer);
                        OpenFold();
                    }
                    ...
                    if (base==s)
                    {
                        ...
                        ExpandAndAugment(firstProp,Q[t]);
                    ...
                    if (CT.logMeth>1)
                    {
                        sprintf(CT.logBuffer,"Shrinking %ld",base);
                    ...
                    if (CT.logMeth>1)
                    {
                        sprintf(CT.logBuffer," into node %ld",z);
            ...
            if (searching)
            {
                ...
                if (epsilon<InfFloat && epsilon>0)
                {
                    ...
                    for (TNode v=0;v<n;v++)
                    {
                        ...
                        if (CT.logMeth>1)
                        {
                            sprintf(CT.logBuffer,"pi[%ld] = %g",v,pi[v]);
                            LogEntry(LOG_METH2,CT.logBuffer);
                        }
    ...
    while (I.Active(v))
    {
        try
        {
            ...
            if (BalCap(a)>0 && (dist[v^1]==InfFloat || prop[v^1]!=(a^2)))
            {
                ...
                {
                    sprintf(CT.logBuffer,
                        "Arc %ld joining %ld and %ld has modified length %g",
                        a,G.StartNode(a),G.EndNode(a),thisLength);
...
void surfaceGraph::PrimalDual1(TNode s) throw(ERRange)
{
    ...
    dynamicStack<TNode,TFloat> Support(n,CT);
    dynamicStack<TNode,TFloat> Swap(n,CT);
    staticQueue<TArc,TFloat> Queue(2*m,CT);
            ...
            while (searching && !Queue.Empty())
            {
                ...
                if (dist[cv]==InfFloat)
                {
                    if (dist[v]==InfFloat)
                    {
                        ...
                        {
                            sprintf(CT.logBuffer,
                                "Prop %ld with end nodes %ld and %ld found",
                                a,u,v);
                            LogEntry(LOG_METH2,CT.logBuffer);
                        }
                ...
                {
                    sprintf(CT.logBuffer,
                        "Petal %ld with end nodes %ld and %ld found",
                        a,u,v);
                    LogEntry(LOG_METH2,CT.logBuffer);
                    OpenFold();
                }
