...
/// \file   ilpWrapper.cpp
/// \brief  #goblinILPWrapper partial class implementastion

#include "ilpWrapper.h"


goblinILPWrapper::goblinILPWrapper(goblinController& thisContext) throw() :
...
goblinILPWrapper::~goblinILPWrapper() throw()
{
    ReleaseVarValues();
...
unsigned long goblinILPWrapper::Allocated() const throw()
{
...
char* goblinILPWrapper::Display() const throw(ERFile,ERRejected)
{
...
char* goblinILPWrapper::UnifiedLabel(TRestr i,TOwnership tp) const throw(ERRange)
{
...
void goblinILPWrapper::WriteLPNaive(const char* fileName,TDisplayOpt opt)
    const throw(ERFile,ERRejected)
{
    ...
    goblinILPModule *ModLP = (goblinILPModule*)CT.pLPModule;
    bool columnOriented =
        (ModLP->Orientation() == goblinILPModule::COLUMN_ORIENTED);
...
void goblinILPWrapper::WriteMPSFile(const char* fileName,TLPFormat f)
    const throw(ERFile,ERRejected)
{
    #if defined(_TIMERS_)
...
void goblinILPWrapper::WriteMPSFile(ofstream& expFile,TLPFormat f)
    const throw(ERFile,ERRejected)
{
    char lineBuffer[64] = "";
    int l1 = 11;
...
void goblinILPWrapper::WriteBASFile(const char* fileName,TLPFormat f)
    const throw(ERFile,ERRejected)
{
    #if defined(_TIMERS_)
...
void goblinILPWrapper::WriteBASFile(ofstream& expFile,TLPFormat f)
    const throw(ERFile,ERRejected)
{
    char lineBuffer[64] = "";
...
void goblinILPWrapper::Write(const char* fileName,TOption opt)
    const throw(ERFile,ERRejected)
{
    #if defined(_TIMERS_)
    ...
    {
        F.MakeIntItem(pivotRow,0);
        F.MakeIntItem(pivotColumn,0);
        F.MakeIntItem(pivotDir,0);
    }
    else F.MakeNoItem(0);
...
void goblinILPWrapper::Write(const char* fileName,TLPFormat f,TOption opt)
    const throw(ERFile,ERRejected)
{
    switch (f)
    {
...
TRestr goblinILPWrapper::ReadRowLabel(char* thisLabel) throw()
{
...
TVar goblinILPWrapper::ReadColLabel(char* thisLabel, bool markInt) throw()
{
...
void goblinILPWrapper::ReadMPSFile(const char* fileName)
...
void goblinILPWrapper::ReadMPSFile(ifstream& impFile) throw(ERParse,ERRejected)
{
    char lineBuffer[200] = "";
...
void goblinILPWrapper::ReadBASFile(const char* fileName)
    throw(ERParse,ERRejected)
{
    #if defined(_TIMERS_)
...
void goblinILPWrapper::ReadBASFile(ifstream& impFile) throw(ERParse,ERRejected)
{
    char lineBuffer[64] = "";
...
void goblinILPWrapper::NoSuchVar(char* methodName,TVar j) const throw(ERRange)
{
...
void goblinILPWrapper::NoSuchRestr(char* methodName,TRestr i) const throw(ERRange)
{
...
char* goblinILPWrapper::VarLabel(TVar i,TOwnership tp) const throw(ERRange)
{
...
char* goblinILPWrapper::RestrLabel(TRestr i,TOwnership tp) const throw(ERRange)
{
...
TVar goblinILPWrapper::VarIndex(char* thisLabel) const throw()
{
    TVar i = 0;
...
TRestr goblinILPWrapper::RestrIndex(char* thisLabel) const throw()
{
    TRestr i = 0;
...
TFloat goblinILPWrapper::ObjVal()
    const throw()
{
    TFloat s = 0;
...
TFloat goblinILPWrapper::VarValue(TVar j)
    const throw(ERRange)
{
    #if defined(_FAILSAVE_)
...
void goblinILPWrapper::SetVarValue(TVar j,TFloat val)
    throw(ERRange)
{
    #if defined(_FAILSAVE_)
...
void goblinILPWrapper::InitVarValues(TFloat defVal)
    throw()
{
    if (!varValue)
    {
...
void goblinILPWrapper::ReleaseVarValues()
    throw()
{
    if (varValue)
    {
...
void goblinILPWrapper::WriteVarValues(goblinExport* F)
    const throw()
{
    if (varValue==NULL)
    {
    ...
    else
    {
        ...
        for (TVar j=0;j<L();j++)
        {
            size_t thisLength = CT.ExternalLength<TFloat>(varValue[j]);
        ...
        for (TVar j=0;j<L();j++)
        {
            if (varValue[j]!=InfFloat)
                F->MakeFloatItem(varValue[j],Length);
...
void goblinILPWrapper::ReadVarValues(goblinImport* F,TVar l)
    throw(ERParse)
{
    ReleaseVarValues();
...
TFloat goblinILPWrapper::Slack(TRestr i,TLowerUpper lu) const throw(ERRange)
{
    #if defined(_FAILSAVE_)
...
bool goblinILPWrapper::PrimalFeasible(TFloat epsilon)
    throw()
{
    for (TRestr i=0;i<K()+L();i++)
    {
...
bool goblinILPWrapper::DualFeasible(TFloat epsilon)
    throw()
{
    for (TVar i=0;i<L();i++)
    {
...
TFloat goblinILPWrapper::SolveLP()
    throw(ERRejected) 
{
    for (TVar j=0;j<L();j++)
    ...
    LogEntry(LOG_METH,"Starting LP solver...");
    OpenFold(ModLpSolve);
    ...
    CloseFold(ModLpSolve);

    return ret;
}


void goblinILPWrapper::AddCuttingPlane() throw(ERRejected)
{
