...
{
    G.MakeRef();
    blocked = new staticQueue<TNode,TFloat>(n,CT);
...
void layeredAuxNetwork::ConfigDisplay(goblinController &CFG) const throw()
{
    CFG.nodeLabels = 1;
    CFG.arcLabels = 2;
    CFG.arcLabels2 = 0;
...
TFloat layeredAuxNetwork::FindPath(TNode t) throw(ERRange,ERRejected)
{
    ...
    if (CT.logMeth>1)
    {
        sprintf(CT.logBuffer,"Augmenting by path (%ld",v);
...
void layeredAuxNetwork::TopErasure(TArc a) throw(ERRange,ERRejected)
{
    ...
    if (CT.logMeth>1)
    {
        sprintf(CT.logBuffer,"Node %ld has indegree %ld",x,currentDegree[x]);
    ...
    if (currentDegree[x]==0)
    {
        ...
        while (!(blocked -> Empty()))
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"Deleting node %ld...",x);
                LogEntry(LOG_METH2,CT.logBuffer);
                OpenFold();
            }
            ...
            for (TArc i=0;i<outDegree[x];i++)
            {
                ...
                if (G.ResCap(aa)>0)
                {
                    ...
                    if (CT.logMeth>1)
                    {
                        sprintf(CT.logBuffer,"Node %ld has indegree %ld",y,
                            currentDegree[y]);
                        LogEntry(LOG_METH2,CT.logBuffer);
                    }
