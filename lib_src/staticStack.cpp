...
char* staticStack<TItem,TKey>::Display() const throw()
{
    ...
    else
    {
        ...
        sprintf(this->CT.logBuffer,"%ld (bottom)",i);
...
template <class TItem,class TKey>
void staticStack<TItem,TKey>::Insert(TItem w,TKey alpha,TOptInsert mode) throw(ERRange,ERCheck)
{
    ...
    if (prev[w]!=n || w==bottom)
    {
        ...
        sprintf(this->CT.logBuffer,"%ld is already on the stack",w);
...
template class staticStack<TNode,TFloat>;

#if defined(_BIG_ARCS_)

template class staticStack<TArc,TFloat>;
