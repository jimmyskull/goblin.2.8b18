...
bool abstractMixedGraph::EdgeSeriesParallelMethod(
    TOptSeriesParallel options,TNode s,TNode t,abstractMixedGraph* _T) throw (ERRejected)
{
    // If not provided by the calling context, compute a binary decomposition tree
    abstractMixedGraph* T = _T;

    if (!T) T = ESP_DecompositionTree(options,s,t);
    ...
    if (!(options & (ESP_VISIBILITY | ESP_ORIENT)))
    {
        ...
        CloseFold(ModSeriesParallel);
        LogEntry(LOG_RES,"...Edge series-parallel embedding found");
    ...
    for (TArc i=2*m-2;i<NoArc;i--)
    {
        ...
        if (C(s,0)<0)
        {
            X -> SetC(s,0,(xOffset[i]+thisWidth/2.0)*CT.nodeSep);
            X -> SetC(s,1,yOffset[i]*CT.nodeSep);

            TNode w = X->InsertThreadSuccessor(s);
            X -> SetC(w,0,thisWidth*CT.nodeSep*0.5);
            X -> SetC(w,1,0);
        }

        if (C(t,0)<0)
        {
            X -> SetC(t,0,(xOffset[i]+thisWidth/2.0)*CT.nodeSep);
            X -> SetC(t,1,(yOffset[i]+thisHeight)*CT.nodeSep);
        ...
        if (i<m)
        {
            ...
            X -> SetC(x,1,C(s,1) + signy*CT.bendSep);
            X -> SetC(y,1,C(t,1) - signy*CT.bendSep);
        }
    }
    ...
    if (options & ESP_ORIENT)
    {
        ...
        LogEntry(LOG_RES,"...Edge series parallel orientation found");
    }

    if (options & ESP_VISIBILITY)
    {
        LogEntry(LOG_RES,"...Edge series parallel drawing found");
    }
...
abstractMixedGraph* abstractMixedGraph::ESP_DecompositionTree(
    TOptSeriesParallel options,TNode s,TNode t) throw ()
{
    LogEntry(LOG_METH,"Computing edge series parallel decomposition tree...");
    OpenFold(ModSeriesParallel);
    ...
    while (!Q.Empty())
    {
        ...
        else if (a3==a1)
        {
            ...
            sprintf(CT.logBuffer,
                "Serial reduction of %li[%li]%li[%li]%li to %li[%li]%li",
                w1,a1^1,v,a2,w2,w1,2*aNew,w2);
            LogEntry(LOG_METH2,CT.logBuffer);

            Q.Insert(w1,INSERT_NO_THROW);
            Q.Insert(w2,INSERT_NO_THROW);
        ...
        if (parallelArc1!=NoArc)
        {
            ...
            sprintf(CT.logBuffer,
                "Parallel reduction of %li[%li,%li]%li to %li[%li]%li",
                v,parallelArc1,parallelArc2,G.EndNode(parallelArc1),v,2*aNew,G.EndNode(parallelArc1));
            LogEntry(LOG_METH2,CT.logBuffer);
    ...
    if (G.M()>m && CT.traceLevel>0)
    {
        ...
        CT.SuppressLogging();
        // ## This work-around is necessary until every object can store its layout parameters
        int savedArcStyle = CT.arcStyle;
        T -> Layout_PredecessorTree();
        CT.arcStyle = savedArcStyle;
        CT.RestoreLogging();
    ...
    if (isSeriesParallel)
    {
        if (G.M()<2*m-1)
        {
            LogEntry(LOG_RES,"...Graph could not be entirely decomposed");
        ...
        {
            if (G.StartNode(4*(m-1))==t) nodeType[2*(m-1)] ^= 1;

            LogEntry(LOG_RES,"...Graph is edge series-parallel");
            return T;
        }
    }
...
{
    LogEntry(LOG_METH,"Extracting forbidden subgraph...");
    OpenFold(ModSeriesParallel);
    ...
    else
    {
        ...
        {
            CloseFold(ModSeriesParallel);
            LogEntry(LOG_RES,"...Reduced graph forms a tree");
            return;
        }
        ...
        for (TArc a=0;a<G.M();a++)
        {
            if ((H2->OriginalArc(2*a)>>1)==(retArc>>1))
            {
                retArcH2 = 2*a;
                break;
            }
        ...
        {
            CloseFold(ModSeriesParallel);
            LogEntry(LOG_RES,"...Node rule to construct minors of non-biconnected graphs");
            return;
        }
    ...
    {
        CloseFold(ModSeriesParallel);
        LogEntry(LOG_RES,"...Graph is recurrent");
        delete H;
        return;
    }
    ...
    {
        CloseFold(ModSeriesParallel);
        LogEntry(LOG_RES,"...Graph has multiple target nodes");
        delete H;
        return;
    }
    ...
    while (u1!=u2)
    {
        ...
        while (true)
        {
            ...
            {
                CloseFold(ModSeriesParallel);
                LogEntry(LOG_RES,"...Graph has multiple source nodes");
                delete H;
                return;
            }
        }
    ...
    if (options & ESP_DIRECTED)
    {
        ...
        {
            G.SetEdgeColour(H->OriginalArc(2*a),H->EdgeColour(2*a));
        }
    ...
    else
    {
        for (TArc a=0;a<H->M();++a)
        {
            TArc a2 = H2->OriginalArc(H->OriginalArc(2*a));
    ...
    // the original arcs
    staticStack<TNode,TFloat> Q(T->N(),CT);
    ...
    CloseFold(ModSeriesParallel);
    LogEntry(LOG_RES,"...Forbidden subgraph found");
}
