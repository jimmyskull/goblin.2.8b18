    ...
    throw(ERRejected)
{
    ...
    switch (arrayType)
    {
        ...
        case TYPE_STRING:
        {
            tuple = static_cast<void*>(new TString[length]);
            break;
        }
        case TYPE_VAR_INDEX:
        {
    ...
    while (!tail)
    {
        ...
        if (strlen(label)>0)
        {
            ...
            switch (arrayType)
            {
                ...
                case TYPE_STRING:
                {
                    static_cast<TString*>(_array)[length++] = TString(label);
                    break;
                }
                case TYPE_VAR_INDEX:
                {
    ...
    throw(ERParse)
{
    ...
    // If this procedure is called as a part of the graph loader, the
    // layout model has been read before, and the configuration part
    // lists only differences with the default layout parameters.
    // Otherwise, the configuration list is considered exhaustive
    // and calling SetDisplayParameters() does not have any effect.

    CT.SetDisplayParameters(TLayoutModel(CT.layoutModel));
