...
void sparseRepresentation::SetCapacity(TNode _n,TArc _m)
    throw(ERRejected)
{
    #if defined(_FAILSAVE_)

    if (_n<nAct || _m<mAct || _n>CT.MaxNode() || 2*_m>CT.MaxArc())
...
TNode sparseRepresentation::InsertArcLabelAnchor(TArc a)
    throw(ERRejected,ERRange)
{
    #if defined(_FAILSAVE_)

    if (a>=2*mAct) NoSuchArc("InsertArcLabelAnchor",a);
    ...
    if (align!=NULL && align[a>>1]!=NoNode)
        Error(ERR_REJECTED,
            "InsertArcLabelAnchor","Redefinition of alignment point");
    ...
    throw(ERRange)
{
    ...
    // Adjust the thread of bend nodes, but only if the
    // implicit orientations of a1 and a2 are different
    if (((a1&1)^(a2^1)) == 0) return;
...
bool sparseRepresentation::ReleaseBendNodes(TArc a)
    throw(ERRange)
{
    #if defined(_FAILSAVE_)

    if (a>=2*mAct) NoSuchArc("ReleaseBendNodes",a);
...
bool sparseRepresentation::ReleaseDoubleBendNodes() throw()
{
    TNode* align = layoutData.GetArray<TNode>(TokLayoutArcLabel);
    TNode* thread = layoutData.GetArray<TNode>(TokLayoutThread);
...
bool sparseRepresentation::ReleaseCoveredBendNodes(canvasBuilder::TPortMode portMode) throw()
{
    TNode* align = layoutData.GetArray<TNode>(TokLayoutArcLabel);
    TNode* thread = layoutData.GetArray<TNode>(TokLayoutThread);
    TDim dimLayout = Dim();
    ...
    bool isModified = ReleaseDoubleBendNodes();
    ...
    for (TArc a=0;a<mAct;a++)
    {
        ...
        if (portMode==canvasBuilder::PORTS_EXPLICIT)
        {
            y = x;
            x = thread[x];
        ...
        do
        {
            ...
            for (TDim i=0;i<Dim();i++)
            {
                if (fabs(C(x,i)-C(y,i))<CT.bendSep/2.0 && fabs(C(u,i)-C(y,i))<CT.bendSep/2.0)
                {
                    ++matchingCoordinates;
                }
            }

            if (matchingCoordinates+1>=dimLayout)
            {
                // Delete y since it is a convex combination of x and u
                Q.Insert(y,-y);
...
bool sparseRepresentation::ReleaseShapeNodes(TNode v)
    throw(ERRange)
{
    #if defined(_FAILSAVE_)

    if (v>=nAct) NoSuchNode("ReleaseShapeNodes",v);
    ...
    throw(ERRange,ERRejected)
{
    ...
    if (SN[ac]==SN[ac^1])
    {
        sprintf(CT.logBuffer,"Arc is a loop: %ld",ac);
