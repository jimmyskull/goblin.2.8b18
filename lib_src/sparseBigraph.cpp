...
biGraph::biGraph(TNode _n1,TNode _n2,goblinController& _CT) throw() :
    managedObject(_CT),
    abstractBiGraph(_n1,_n2,TArc(0)),
    X(static_cast<const biGraph&>(*this))
{
    X.SetCDemand(1);
...
biGraph::biGraph(const char* fileName,goblinController& _CT) throw(ERFile,ERParse) :
    managedObject(_CT),
...
biGraph::biGraph(abstractBiGraph &G,TOption options) throw() :
...
unsigned long biGraph::Size() const throw()
{
    return
          sizeof(biGraph)
...
unsigned long biGraph::Allocated() const throw()
{
    return 0;
}


TNode biGraph::SwapNode(TNode u) throw(ERRange,ERRejected)
{
...
inducedBigraph::inducedBigraph(abstractMixedGraph& G,
    ...
    managedObject(G.Context()),
    biGraph(TNode(1),TNode(0),G.Context())
{
    X.SetCapacity(G.N(),G.M());

    TIndex* originalNode = (options & OPT_MAPPINGS) ? new TNode[G.N()] : NULL;
    TIndex* originalArc  = (options & OPT_MAPPINGS) ? new TArc[G.M()]  : NULL;
