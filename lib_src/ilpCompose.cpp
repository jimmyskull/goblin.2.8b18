...
goblinILPWrapper* goblinILPWrapper::Clone() throw(ERRejected)
{
    goblinILPModule* ModLP = (goblinILPModule*)CT.pLPModule;
    goblinILPWrapper* XLP  =
        ModLP->NewInstance(K(),L(),NZ(),ObjectSense(),CT);
...
goblinILPWrapper* goblinILPWrapper::DualForm() throw(ERRejected)
{
    ...
    goblinILPModule* ModLP = (goblinILPModule*)CT.pLPModule;
    goblinILPWrapper* XLP  =
        ModLP->NewInstance(L(),2*K(),NZ(),MAXIMIZE,CT);
...
goblinILPWrapper* goblinILPWrapper::StandardForm() throw(ERRejected)
{
    goblinILPModule* ModLP = (goblinILPModule*)CT.pLPModule;
    goblinILPWrapper* XLP  =
        ModLP->NewInstance(2*K()+L()+1,2*K()+L()+1,2*(NZ()+L()),ObjectSense(),CT);
...
goblinILPWrapper* goblinILPWrapper::CanonicalForm() throw(ERRejected)
{
    goblinILPModule* ModLP = (goblinILPModule*)CT.pLPModule;
    goblinILPWrapper* XLP  =
        ModLP->NewInstance(2*(K()+L()),L(),2*(NZ()+L()),ObjectSense(),CT);
...
void goblinILPWrapper::FlipObjectSense() throw()
{
    if (ObjectSense()==MAXIMIZE)
    {
...
managedObject* abstractMixedGraph::BFlowToLP() throw()
{
    ...
    goblinILPWrapper *XLP  =
        ModLP->NewInstance(n,m,2*m,goblinILPWrapper::MINIMIZE,CT);
...
managedObject* abstractMixedGraph::VerticalCoordinatesModel(TNode* nodeLayer) throw()
{
    ...
    goblinILPModule *ModLP =
        (goblinILPModule*)goblinController::pLPModule;
    goblinILPWrapper *XLP  =
        ModLP->NewInstance(m,n,2*m,goblinILPWrapper::MINIMIZE,CT);
    ...
    for (TVar v=0;v<TVar(n);v++)
    {
        ...
        {
            XLP -> AddVar(nodeLayer[v],nodeLayer[v],div,goblinILPWrapper::VAR_FLOAT);
        }
        else
        {
            XLP -> AddVar(0,InfFloat,div,goblinILPWrapper::VAR_FLOAT);
        }
    }
...
managedObject* abstractMixedGraph::HorizontalCoordinatesModel() throw()
{
    goblinILPModule *ModLP =
        (goblinILPModule*)goblinController::pLPModule;
    goblinILPWrapper *XLP  =
        ModLP->NewInstance(2*m+n,n+m,2*m,goblinILPWrapper::MINIMIZE,CT);
    ...
    for (TVar v=0;v<TVar(n);v++)
    {
        ...
        XLP -> AddVar(0,InfFloat,thisLength,goblinILPWrapper::VAR_FLOAT);
    }

    for (TArc a=0;a<m;a++)
    {
        // Add a variable for every graph edge. Use the original length label
        XLP -> AddVar(0,InfFloat,-2*Length(2*a),goblinILPWrapper::VAR_FLOAT);
    }
    ...
    for (TVar v=0;v<TVar(n);v++)
    {
        ...
        for (TVar w=0;w<TVar(n);w++)
        {
            ...
            // Due to the fact that Layout_SubdivideArcs() adds bend nodes
...
managedObject* abstractMixedGraph::StableSetToMIP() throw()
{
    ...
    goblinILPModule* ModLP = (goblinILPModule*)CT.pLPModule;
    goblinILPWrapper* XLP  =
        ModLP->NewInstance(m-mShrunk+nColours,n,2*m,MAXIMIZE,CT);


    for (TVar i=0;i<TVar(n);i++)
    {
        XLP -> AddVar(0,1,1,goblinILPWrapper::VAR_INT);
    }
