    ...
    throw (ERRejected)
{
    ...
    staticStack<TNode,TFloat> Heads(n,CT);
    staticStack<TNode,TFloat> Tails(n,CT);
    ...
    if (!preserveEmbedding)
    {
        ...
        for (TNode r=0;r<n;r++)
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"Component %ld:",l);
            ...
            while (!Q.Empty())
            {
                ...
                if (CT.logMeth>1)
                {
                    sprintf(CT.logBuffer,"Face %ld = (%ld [%ld] %ld",
                        k,w,a2,v);
                ...
                while (a2!=a)
                {
                    ...
                    if (CT.logMeth>1)
                    {
                        sprintf(CT.logBuffer," [%ld] %ld",a2,v);
                    ...
                    if (!preserveExteriorFace)
                    {
                        if (CT.logMeth>1)
                        {
                            sprintf(CT.logBuffer,
                                "...Exterior face %ld has %ld nodes",
                                face[maxRegion],maxLength);
        ...
        sprintf(CT.logBuffer,
            "...Graph has %ld faces and %ld connected components",
            k-l+1,l);
        LogEntry(LOG_RES,CT.logBuffer);
    }
    else
    {
        ...
        sprintf(CT.logBuffer,"...Graph has %ld faces",ND());
        LogEntry(LOG_RES,CT.logBuffer);
        sprintf(CT.logBuffer,"...Exterior face %ld has %ld nodes",
            face[exteriorArc],l);
        LogEntry(LOG_RES,CT.logBuffer);
    }
...
TNode abstractMixedGraph::GrowExteriorFace(TArc exteriorArc) throw(ERRejected)
{
    ...
    LogEntry(LOG_METH,"Growing exterior face...");
    OpenFold();
    ...
    while (a!=exteriorArc || blocked)
    {
        ...
        Q.Insert(x,INSERT_NO_THROW);

        #if defined(_LOGGING_)

        if (CT.logMeth>1)
        {
            sprintf(CT.logBuffer," [%ld] %ld",aNext,y);
            LogAppend(LH,CT.logBuffer);
        }
    ...
    if (!outerplanar)
    {
        sprintf(CT.logBuffer,"...Exterior region has %ld nodes",Q.Cardinality());
        LogEntry(LOG_RES,CT.logBuffer);
    }
    else
    {
        if (Q.Cardinality()==n) LogEntry(LOG_RES,"...Graph is outerplanar");
        else LogEntry(LOG_RES,"...Component is outerplanar");
    }
    ...
    throw(ERRejected)
{
    ...
    if (!ExteriorNode(x))
    {
        sprintf(CT.logBuffer,"Not an exterior node: %ld",x);
    ...
    {
        sprintf(CT.logBuffer,"...%ld blocks have been moved",moved);
        LogEntry(LOG_RES,CT.logBuffer);
    }
...
void abstractMixedGraph::PlanarBiconnectivityAugmentation() throw (ERRejected)
{
    ...
    LogEntry(LOG_METH,"Augmenting to a biconnected planar graph...");
    OpenFold();
    OpenFold();
    ...
    for (TArc a=0;a<2*mOrig;a++)
    {
        ...
        {
            CloseFold();
            sprintf(CT.logBuffer,"Tracing face of arc %ld...",a);
            LogEntry(LOG_METH2,CT.logBuffer);
            OpenFold();
        }
        ...
        do
        {
            ...
            if (a3==(a2^1))
            {
                ...
                else
                {
                    ...
                    if (CT.logMeth>1)
                    {
                        sprintf(CT.logBuffer,"Adding arc (%ld,%ld)",
                            threadNode,v);
                        LogEntry(LOG_METH2,CT.logBuffer);
                    }
...
void abstractMixedGraph::Triangulation() throw (ERRejected)
{
    ...
    LogEntry(LOG_METH,"Triangulating graph...");
    OpenFold();
    OpenFold();
    ...
    for (TArc a=0;a<2*mOrig;a++)
    {
        ...
        // Do not proceed only if the face left hand of a is already a triangle
        if (l<4) continue;
        ...
        while (true)
        {
            ...
            else
            {
                ...
                if (CT.logMeth>1)
                {
                    sprintf(CT.logBuffer,"Adding arc (%ld,%ld)",u,v);
                    LogEntry(LOG_METH2,CT.logBuffer);
                }
...
TNode abstractMixedGraph::LMCOrderedPartition(
    ...
    throw (ERRejected)
{
    LogEntry(LOG_METH,"Computing LMC ordered partition...");
    OpenFold(ModLMCOrder);

    #if defined(_PROGRESS_)

    InitProgressCounter(3*n);
    ...
    for (TNode f=0;f<nd;f++)
    {
        ...
        if (CT.logMeth>1)
        {
            sprintf(CT.logBuffer,"vOut[%ld] = %ld, aOut[%ld] = %ld",
                f,vOut[f],f,aOut[f]);
            LogEntry(LOG_METH2,CT.logBuffer);
        }
    ...
    staticStack<TNode,TFloat> nodeQueue(n,CT);
    ...
    staticStack<TArc,TFloat> segmentQueue(2*m,CT);

    // Keep all faces which have become a separating face in an iteration
    staticStack<TArc,TFloat> newSeparatingFaces(2*m,CT);
    ...
    while (connected && G.Right(aBasis^1,v1)!=(aBasis^1))
    {
        ...
        if (!nodeQueue.Empty())
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"Component %ld (Node): %ld",k,vDel);
                LogEntry(LOG_METH2,CT.logBuffer);
            }
            ...
            do
            {
                ...
                if (w!=v1 && w!=v2) nodeQueue.Insert(w,INSERT_NO_THROW);
        ...
        else if (!segmentQueue.Empty())
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"Component %ld (Path): %ld",k,vDel);
                LH = LogStart(LOG_METH2,CT.logBuffer);
            }
            ...
            if (cl!=v1 && fSep[cl]==0) nodeQueue.Insert(cl,INSERT_NO_THROW);

            if (cr!=v2 && fSep[cr]==0) nodeQueue.Insert(cr,INSERT_NO_THROW);
        }
        else
        {
            connected = false;
        ...
        if (vOut[f]==2 && aOut[f]==0 && v==cr)
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"Not a separator face: %ld",f);
                LogEntry(LOG_METH2,CT.logBuffer);
            }
        ...
        if (CT.logMeth>1)
        {
            sprintf(CT.logBuffer,"New exterior path: (%ld",cl);
            LH = LogStart(LOG_METH2,CT.logBuffer);
        }
        ...
        while (u!=cr)
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer," [%ld] %ld",a,v);
                LogAppend(LH,CT.logBuffer);
            }
        ...
        while (!newSeparatingFaces.Empty())
        {
            ...
            if (vOut[f]>aOut[f]+1 || aOut[f]>1)
            {
                ...
                if (CT.logMeth>1)
                {
                    sprintf(CT.logBuffer,"New separator face: %ld",f);
                    LogEntry(LOG_METH2,CT.logBuffer);
                }
        ...
        for (TNode v=0;v<n;v++)
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"fSep[%ld] = %ld",
                    v,fSep[v]);
                LogEntry(LOG_METH2,CT.logBuffer);
            }
        }
    ...
    {
        sprintf(CT.logBuffer,"Component %ld (Basis): %ld,%ld",k,v1,v2);
        LogEntry(LOG_METH2,CT.logBuffer);
    }
    ...
    if (connected)
    {
        ...
        staticStack<TNode,TFloat>** partByRight
            = new staticStack<TNode,TFloat>*[n];
        partByRight[0] = new staticStack<TNode,TFloat>(n,CT);

        for (TNode v=1;v<n;v++)
            partByRight[v] = new staticStack<TNode,TFloat>(*partByRight[0]);
        ...
        if (CT.logMeth>1)
        {
            sprintf(CT.logBuffer,"Component: %ld",k-1);
            LH = LogStart(LOG_METH2,CT.logBuffer);
        }
    ...
    if (connected)
    {
        LogEntry(LOG_RES, "...Graph has been decomposed");
    }
    else
    {
        LogEntry(LOG_RES, "...Graph is not 3-connected");
...
void abstractMixedGraph::Layout_StraightLineDrawing(TArc aBasis,int spacing)
    throw(ERRejected)
{
    #if defined(_FAILSAVE_)
    ...
    CloseFold();
    LogEntry(LOG_RES, "...Straight line drawing found");

    CloseFold(ModPlanarity,NO_INDENT);
}


void abstractMixedGraph::Layout_ConvexDrawing(TArc aBasis,int spacing)
    throw(ERRejected)
{
    #if defined(_FAILSAVE_)
    ...
    OpenFold(ModConvexDrawing,NO_INDENT);
    ...
    {
        CloseFold();
        CloseFold(ModConvexDrawing,NO_INDENT);
    ...
    // Only for display: Hide all nodes initially
    for (TNode v=0;v<n;v++) SetC(v,0,-InfFloat);
    ...
    for (TNode l=1;l<k;l++)
    {
        ...
        if (CT.traceLevel>2)
        {
            ...
            for (v=0;v<n;v++)
            {
                ...
                if (thisShift>0)
                {
                    SetC(v,0,C(v,0)+thisShift*sep);

                    #if defined(_LOGGING_)

                    if (CT.logMeth>1)
                    {
                        sprintf(CT.logBuffer,"x[%ld] += %g",
                            v,thisShift*sep);
                        LogEntry(LOG_METH2,CT.logBuffer);
                    }
        ...
        if (CT.traceLevel<3)
        {
            ...
            while (v!=vr)
            {
                ...
                if (v==vr)
                {
                    shift[v] += sumShift+2*q*sep;

                    #if defined(_LOGGING_)

                    if (CT.logMeth>1)
                    {
                        sprintf(CT.logBuffer,"x[%ld] += %g + %g",
                            v,sumShift,2*q*sep);
                        LogEntry(LOG_METH2,CT.logBuffer);
                    }
                ...
                else
                {
                    ...
                    {
                        sprintf(CT.logBuffer,"x[%ld] += %g",v,sumShift);
                        LogEntry(LOG_METH2,CT.logBuffer);
                    }
            ...
            xl = xCurrent[vl];
            xr = xCurrent[vr]+shift[vr]-2*(q-1)*sep;
        }
        ...
        while (vNext!=NoNode)
        {
            ...
            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"Insert node %ld at (%g,%g)",
                    vNext,xNew,yNew);
                LogEntry(LOG_METH2,CT.logBuffer);
            }
    ...
    if (CT.traceLevel<3)
    {
        ...
        for (TNode l=k-1;l>0;l--)
        {
            ...
            while (vNext!=NoNode)
            {
                ...
                if (CT.logMeth>1)
                {
                    sprintf(CT.logBuffer,"x[%ld] = %g + %g + %g",
                        vNext,xInsert[vNext],shift[vNext],rShift[vNext]);
                    LogEntry(LOG_METH2,CT.logBuffer);
                }
            ...
            do
            {
                ...
                if (layer[v]<=layer[w] || w==EndNode(cLeft[l]))
                {
                    ...
                    {
                        shift[v] = shift[EndNode(cLeft[l])]+sumRShift+q*sep;

                        #if defined(_LOGGING_)

                        if (CT.logMeth>1)
                        {
                            sprintf(CT.logBuffer,"shift[%ld] = %g + %g + %g",
                                v,shift[EndNode(cLeft[l])],sumRShift,q*sep);
                            LogEntry(LOG_METH2,CT.logBuffer);
                        }
                ...
                else if (v!=vl && v!=vr)
                {
                    ...
                    if (CT.logMeth>1)
                    {
                        sprintf(CT.logBuffer,"shift[%ld] = %g",
                            v,shift[StartNode(cLeft[l])]);
                        LogEntry(LOG_METH2,CT.logBuffer);
                    }
            ...
            rShift[vr] += sumRShift+2*q*sep;

            #if defined(_LOGGING_)

            if (CT.logMeth>1)
            {
                sprintf(CT.logBuffer,"rShift[%ld] += %g + %g",
                    v,sumRShift,2*q*sep);
                LogEntry(LOG_METH2,CT.logBuffer);
            }
            ...
            if (pointUp!=NoArc)
            {
                ...
                while (v!=vr)
                {
                    ...
                    if (CT.logMeth>1)
                    {
                        sprintf(CT.logBuffer,"shift[%ld] = %g + %g",
                            v,shift[vr],rShift[vr]);
                        LogEntry(LOG_METH2,CT.logBuffer);
                    }
        ...
        if (CT.logMeth>1)
        {
            sprintf(CT.logBuffer,"x[%ld] = %g",v2,rShift[v2]);
            LogEntry(LOG_METH2,CT.logBuffer);
        }
    ...
    sprintf(CT.logBuffer,"...Grid size: %ldx%ld",
        TNode(C(v2,0)),TNode(C(EndNode(cLeft[k-1]),1)));
    LogEntry(LOG_RES,CT.logBuffer);
    ...
    for (TNode v=0;v<n;v++) SetC(v,1,-C(v,1));

    CloseFold();
    LogEntry(LOG_RES, "...Convex drawing found");
    CloseFold(ModConvexDrawing,NO_INDENT);
}
