        ...
        for (TItem i=0;i<n;i++)
        {
            ...
            else
            {
                ...
                while (x!=UNDEFINED)
                {
                    ...
                    {
                        sprintf(this->CT.logBuffer,"Inconsistent item: %ld",x);
        ...
        if (card>0)
        {
            for (TItem i=0;i<k;i++)
            {
                if (bucket[i]!=UNDEFINED)
                {
                    ...
                    {
                        sprintf(this->CT.logBuffer,"Inconsistent bucket: %ld",i);
                        InternalError1("Display");
                    }
...
void fibonacciHeap<TItem,TKey>::Display(TItem v) const throw(ERRange)
{
    ...
    sprintf(this->CT.logBuffer,"%ld[%g]",v,key[v]);
...
template class fibonacciHeap<TArc,TFloat>;
