...
public:
    ...
    /// \brief  Skip the first unvisited arc in a node incidence list
    ///
    /// \param v  A node index ranged [0,1,..,n-1]
    ///
    /// This marks the currently indexed arc a in the incidence list of node
    /// v as visited, Technically, the hidden iterator for this incidence list
    /// is positioned to the arc Right(a,v).
    virtual void    Skip(TNode v) throw(ERRange,ERRejected) {Read(v);};
