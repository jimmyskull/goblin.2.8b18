...
{
    goblinILPWrapper* XLP = (goblinILPWrapper*)clientData;
    ...
    else try
    {
        ...
        if (strcmp(argv[1],"write")==0)
        {
            ...
            goblinILPWrapper::TLPFormat f = goblinILPWrapper::GOB_FORMAT;

            if (argc>3)
            {
                if (strcmp(argv[2],"lp")==0) f = goblinILPWrapper::LP_FORMAT; 
                if (strcmp(argv[2],"mps")==0) f = goblinILPWrapper::MPS_FORMAT;
                if (strcmp(argv[2],"cplex")==0) f = goblinILPWrapper::MPS_CPLEX;
                if (strcmp(argv[2],"bas")==0) f = goblinILPWrapper::BAS_CPLEX;
                if (strcmp(argv[2],"basis")==0) f = goblinILPWrapper::BAS_CPLEX;
            }
        ...
        if (strcmp(argv[1],"#rows")==0 || strcmp(argv[1],"#restrictions")==0)
        {
            ...
            sprintf(interp->result,"%ld",XLP->K());
            return TCL_OK;
        }
        ...
        if (strcmp(argv[1],"#columns")==0 || strcmp(argv[1],"#variables")==0)
        {
            ...
            sprintf(interp->result,"%ld",XLP->L());
            return TCL_OK;
        }
        ...
        if (strcmp(argv[1],"orientation")==0)
        {
            ...
            if (ModLP->Orientation()==goblinILPModule::ROW_ORIENTED)
                interp->result = "row";
        ...
        if (strcmp(argv[1],"set")==0)
        {
            ...
            if (strcmp(argv[2],"index")==0)
            {
                ...
                if (strcmp(argv[5],"upper")==0)
                    XLP -> SetIndex(i,j,goblinILPWrapper::UPPER);
                else XLP -> SetIndex(i,j,goblinILPWrapper::LOWER);
        ...
        if (strcmp(argv[1],"pivot")==0)
        {
            ...
            {
                TRestr i = XLP-> PivotRow();
                goblinILPWrapper::TLowerUpper tp = XLP-> PivotDirection();

                if (i==NoRestr) interp->result = "";
                else
                {
                    if (tp==goblinILPWrapper::LOWER)
                        interp->result = "lower";
            ...
            goblinILPWrapper::TLowerUpper tp = goblinILPWrapper::UPPER;

            if (strcmp(argv[4],"lower")==0) tp = goblinILPWrapper::LOWER;
        ...
        if (strcmp(argv[1],"dual")==0)
        {
            ...
            goblinILPWrapper* YLP = XLP -> DualForm();

            Tcl_CreateCommand(interp,const_cast<char*>(argv[2]),
                Goblin_Ilp_Cmd,(ClientData)YLP,
        ...
        if (strcmp(argv[1],"standard")==0)
        {
            ...
            goblinILPWrapper* YLP = XLP -> StandardForm();
        ...
        if (strcmp(argv[1],"canonical")==0)
        {
            ...
            goblinILPWrapper* YLP = XLP -> CanonicalForm();
...
int Goblin_Row_Cmd (goblinILPWrapper *XLP,Tcl_Interp* interp,int argc,
    _CONST_QUAL_ char* argv[]) throw(ERRejected,ERRange)
{
    ...
    {
        if (XLP->RestrType(i)==goblinILPWrapper::NON_BASIC)
            interp->result = "non_basic";
        if (XLP->RestrType(i)==goblinILPWrapper::BASIC_UB)
            interp->result = "upper";
        if (XLP->RestrType(i)==goblinILPWrapper::BASIC_LB)
            interp->result = "lower";
        if (XLP->RestrType(i)==goblinILPWrapper::RESTR_CANCELED)
    ...
    if (strcmp(argv[3],"value")==0)
    {
        ...
        {
            sprintf(interp->result,"%g",XLP->Y(i,goblinILPWrapper::LOWER));
            return TCL_OK;
        }

        if (strcmp(argv[4],"upper")==0)
        {
            sprintf(interp->result,"%g",XLP->Y(i,goblinILPWrapper::UPPER));
            return TCL_OK;
        }
    ...
    if (strcmp(argv[3],"slack")==0)
    {
        ...
        if (strcmp(argv[4],"lower")==0)
        {
            sprintf(interp->result,"%g",XLP->Slack(i,goblinILPWrapper::LOWER));
            return TCL_OK;
        }

        if (strcmp(argv[4],"upper")==0)
        {
            sprintf(interp->result,"%g",XLP->Slack(i,goblinILPWrapper::UPPER));
            return TCL_OK;
        }
...
int Goblin_Variable_Cmd (goblinILPWrapper *XLP,Tcl_Interp* interp,int argc,
    _CONST_QUAL_ char* argv[]) throw(ERRejected,ERRange)
{
    if (argc<4)
    {
    ...
    if (strcmp(argv[2],"insert")==0)
    {
        ...
        goblinILPWrapper::TVarType tp = goblinILPWrapper::VAR_FLOAT;

        if (strcmp(argv[3],"integer")==0) tp = goblinILPWrapper::VAR_INT;
        else
        {
    ...
    if (strcmp(argv[3],"type")==0)
    {
        if (XLP->VarType(i)==goblinILPWrapper::VAR_INT)
            interp->result = "integer";
        if (XLP->VarType(i)==goblinILPWrapper::VAR_FLOAT)
            interp->result = "float";
        if (XLP->VarType(i)==goblinILPWrapper::VAR_CANCELED)
            interp->result = "canceled";
    ...
    if (strcmp(argv[3],"mark")==0)
    {
        ...
        {
            XLP -> SetVarType(i,goblinILPWrapper::VAR_INT);
            interp->result = "";
            return TCL_OK;
        }
        ...
        {
            XLP -> SetVarType(i,goblinILPWrapper::VAR_FLOAT);
            interp->result = "";
            return TCL_OK;
        }
