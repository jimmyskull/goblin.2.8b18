...
#   manual      Generate the reference manual mgoblin.<version>.ps and .pdf
#   pkg         Generate a source code tarball goblin.<version>.tgz
#   bin         Generate a binary distribution package
...
vflags = -D_GOBLIN_MAJOR_VERSION_=$(major_version) \
         -D_GOBLIN_MINOR_VERSION_=$(minor_version) \
         -D_GOBLIN_REVISION_="\""$(revision)"\""
...
$(lib_src_path)/goblinController.o : goblinController.cpp $(rel_depended)
	$(CXX) $(CXXFLAGS) -I$(lib_incl_path) $(vflags) -c $< -o $@
	@echo -n .
...
.SILENT : clean
clean : intro
	rm -f $(OBJS) try.o try libgoblin.a libglpk.a libgoblin.$(so_suffix) libglpk.$(so_suffix)
...
pkg : intro
	...
	cp samples/samples.ref $(rname)/samples
	mkdir $(ppath)
...
.PHONY : bin
.SILENT : bin
bin : shell $(static_archives) $(shared_objects) $(iscript) $(uscript) 
...
ifneq ($(link_mode),static)
	cp libgoblin.$(so_suffix) lib
ifdef glpk
	cp libglpk.$(so_suffix) lib
endif
endif
	mkdir -p bin
ifneq ($(link_mode),dynamic)
	cp gosh$(exe_suffix) bin
